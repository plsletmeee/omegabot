...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(colour) if((!colour == "Red" && !colour.startsWith('#')) || (!colour.startsWith('#') && !colour == "Red")) return interaction.reply({content: "<:cross:1062133327370399884> An error occured. You may have used some invalid information.", ephemeral: true})
        if(colour) if(colour.startsWith('#') && colour.length !== 7) return interaction.reply({content: "<:cross:1062133327370399884> An error occured. You may have used some invalid information.", ephemeral: true})
        ...
        await interaction.channel.send({embeds: [embed]}).catch(() => {return interaction.reply({content: "<:cross:1062133327370399884> An error occured. You may have used some invalid information.", ephemeral: true})})
