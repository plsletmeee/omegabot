...
module.exports = {
    ...
    async execute(interaction, premium) {
        ...
        if (interaction.type === InteractionType.ApplicationCommand) {
            const command = premium.commands.get(interaction.commandName);
        
            await command.execute(interaction, premium).catch((err) => {
        ...
        if (interaction.isButton()) {
            ...
            if(interaction.customId === "messageDelete") {
                await premium.channels.fetch(interaction.channelId, true);
