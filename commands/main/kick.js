const { EmbedBuilder, ApplicationCommandOptionType, PermissionFlagsBits } = require('discord.js');

module.exports = {
    name: 'kick',
    description: 'Kick a member from your server',
    defaultMemberPermissions: PermissionFlagsBits.KickMembers,
    options: [
        {
            name: "member",
            description: "Who you want to kick",
            type: ApplicationCommandOptionType.User,
            required: true
        },
        {
            name: "reason",
            description: "Why you're kicking them",
            type: ApplicationCommandOptionType.String,
            required: false
        },
        {
            name: "evidence",
            description: "Evidence of what they did",
            type: ApplicationCommandOptionType.Attachment,
            required: false
        }
    ],
    async execute(interaction) {

        const member = interaction.options.getMember('member');
        const reason = interaction.options.getString('reason');
        const evidence = interaction.options.getAttachment('evidence');

        let evidenceLink = undefined;
        if(evidence !== null) evidenceLink = `[${evidence.name}](${evidence.attachment})`;

        const embed = new EmbedBuilder()
        .setColor("#2f3136")
        .setAuthor({name: `${member.user.username}`, iconURL: `${member.displayAvatarURL({dynamic: true})}`})
        .setDescription(`**__Member Kicked__**\n**Name:** ${member.user.tag}\n**ID:** ${member.user.id}\n**Joined:** <t:${parseInt(member.joinedTimestamp / 1000)}:R>\n**Evidence:** ${evidenceLink || "None"}\n**Reason:** ${reason || "None"}`)

        if(!member.permissions.has(PermissionFlagsBits.Administrator)) {
            member.kick(reason);
            interaction.reply({embeds: [embed]});
        } else if(member.permissions.has(PermissionFlagsBits.Administrator)) {
            interaction.reply({content: "<a:obcross:1018078642607239218> You cannot kick a member with the Administrator permission using this command!", ephemeral: true});
        }

    }
}