...
router.post('/1', async (req, res) => {
    ...
    const authHeader = req.get('Authorization'); if(!authHeader) return res.status(401).send({ message: 'Missing Authorization Header', code: 401 }), console.log('1')
    const apiResponse = await fetch(`${process.env.URL}/api/data/auth/${authHeader.split(' ')[1]}`); if(apiResponse.status != 200) return res.status(401).send({ message: 'Invalid Authorization Header', code: 401 }), console.log('2')
    const { identity, guilds } = await apiResponse.json()
